...
**当前 PC 模拟器仓库**（`lv_port_pc_vscode`）**无真实 Sensor/编码器/多层显示**，主界面「预览区」为 **占位布局与手势**；落地目标机时，应按本文在 **BSP / 显示驱动 / 采集中间件** 侧实现，LVGL 侧保持 **薄 UI + 命令下发**（与 **`src/ui/ui_hw_hal.c`** 等占位 HAL 的演进方向一致）。
